...
class TaskControllerTest extends WebTestCaseTest
{
    ...
    {
        $client = $this->createClient();
        $client->request('POST', '/task', array('model' => json_encode(array('title' => 'Test unit', 'description' => 'Task use in test unit', 'username' => 'Username unit', 'priority' => 0, 'status' => 0))));
    ...
    {
        $client = $this->createClient();
        $client->request("PUT", "/task/random",  array('model' => json_encode(array('id' => 'random', 'title' => 'Test unit has been updated', 'description' => 'description', 'username' => 'Username unit', 'priority' => 0, 'status' => 0))));
	$this->assertEquals(404, $client->getResponse()->getStatusCode());
        $this->assertEquals('{"error":"Task not found"}', $client->getResponse()->getContent());
    }
    ...
    {
        $client = $this->createClient();
        $client->request("PUT", "/task/530f79b2d58c90be0a0041a7", array('model' => json_encode(array('id' => '530f79b2d58c90be0a0041a7' , 'title' => 'Test unit has been updated', 'description' => 'description', 'username' => 'Username test', 'priority' => 0, 'status' => 0))));
        $this->assertEquals(404, $client->getResponse()->getStatusCode());
        $this->assertEquals('{"error":"Task not found"}', $client->getResponse()->getContent());
    }
    ...
    {
        $client = $this->createClient();
        $client->request('POST', '/task', array('model' => json_encode(array('title' => 'Test unit', 'description' => 'Task use in test unit', 'username' => 'Username test', 'priority' => 0, 'status' => 0))));
        $this->assertEquals($client->getResponse()->getStatusCode(), 200);
